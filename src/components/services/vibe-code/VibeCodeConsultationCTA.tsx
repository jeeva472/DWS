...
import { ArrowRight, Sparkles, Rocket, CheckCircle2, ShieldCheck, Clock } from "lucide-react";
import { ServicesCTAData } from "@/lib/types/wordpress";

interface VibeCodeConsultationCTAProps {
  cta?: ServicesCTAData;
  onOpenConsultation?: () => void;
}

export function VibeCodeConsultationCTA({ cta, onOpenConsultation }: VibeCodeConsultationCTAProps) {
  const shouldReduceMotion = useReducedMotion();
  const eyebrow = cta?.eyebrow || "RAPID BUILD SPRINT";
  const title = cta?.title || "Have an idea you'd like to build quickly?";
  const description =
    cta?.description ||
    "Share your product concept with us. We will evaluate how AI velocity and senior engineering can bring your vision to life rapidly.";
  const buttonText = cta?.buttonText || "Let's Build It";
        ...
        >
          ...
          <div className="mt-12 pt-8 border-t border-white/[0.08] grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-[#7d9287]">
            <div className="flex items-center justify-center gap-2">
              ...
              <span>1-2 Week MVP Turnaround</span>
            </div>
            <div className="flex items-center justify-center gap-2">
              <ShieldCheck className="w-4 h-4 text-[#9ae64c]" />
              <span>Senior Code Quality Guarantee</span>
            </div>
            <div className="flex items-center justify-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-[#9ae64c]" />
              <span>Production-Ready Next.js & React</span>
            </div>
          </div>
        </motion.div>
      </div>
