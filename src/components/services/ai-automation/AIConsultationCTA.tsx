...
import { ArrowRight, Sparkles, MessageSquare, Clock, ShieldCheck } from "lucide-react";
...
export function AIConsultationCTA({ cta, onOpenConsultation }: AIConsultationCTAProps) {
  const shouldReduceMotion = useReducedMotion();
  const eyebrow = cta?.eyebrow || "READY TO AUTOMATE?";
  const title = cta?.title || "What could your business automate?";
  const description =
    cta?.description ||
    "Tell us about the repetitive tasks slowing your team down. We will map a practical automation roadmap tailored to your tools.";
  const buttonText = cta?.buttonText || "Talk to Us";

  return (
    <section id="contact" className="py-24 sm:py-32 bg-[#030608] relative overflow-hidden">
        ...
        >
          ...
          <div className="mt-12 pt-8 border-t border-white/[0.08] grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-[#7d9287]">
            <div className="flex items-center justify-center gap-2">
              ...
              <span>Fast 1-3 Week Deployment</span>
            </div>
            <div className="flex items-center justify-center gap-2">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <span>No Disruption to Current Tools</span>
            </div>
          </div>
        </motion.div>
      </div>
