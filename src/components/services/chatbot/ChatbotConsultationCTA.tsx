...
import { ArrowRight, Sparkles, Bot, CheckCircle2, ShieldCheck, Clock } from "lucide-react";
import { ServicesCTAData } from "@/lib/types/wordpress";

interface ChatbotConsultationCTAProps {
  cta?: ServicesCTAData;
  onOpenConsultation?: () => void;
}

export function ChatbotConsultationCTA({
  cta,
  onOpenConsultation,
}: ChatbotConsultationCTAProps) {
  const shouldReduceMotion = useReducedMotion();
  const eyebrow = cta?.eyebrow || "CONVERSATIONAL AI";
  const title = cta?.title || "Build a Smarter Customer Experience";
  const description =
    cta?.description ||
    "Deploy an intelligent AI assistant that answers questions accurately and captures qualified leads 24/7.";
  const buttonText = cta?.buttonText || "Build a Chatbot";
        ...
        >
          ...
          <div className="mt-12 pt-8 border-t border-white/[0.08] grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-[#7d9287]">
            <div className="flex items-center justify-center gap-2">
              ...
              <span>Instant 24/7 Responsiveness</span>
            </div>
            <div className="flex items-center justify-center gap-2">
