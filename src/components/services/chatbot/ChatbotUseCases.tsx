...
interface ChatbotUseCasesProps {
  useCases?: {
    eyebrow?: string;
    title: string;
    description: string;
...
export function ChatbotUseCases({ useCases }: ChatbotUseCasesProps) {
  const shouldReduceMotion = useReducedMotion();
  const [activeCard, setActiveCard] = useState<number | null>(null);

  const eyebrow = useCases?.eyebrow || "USE CASES";
  const title =
    useCases?.title || "Conversational AI tailored to your business data.";
  const description =
    useCases?.description ||
    "Custom AI assistants connected directly to your documentation, CRM, and customer workflows.";
  const items = useCases?.items || [];

  return (
    <section id="chatbot-use-cases" className="py-24 sm:py-32 bg-[#05080a] relative border-t border-white/[0.06] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, index) => {
            const IconComponent = CHATBOT_ICONS[index % CHATBOT_ICONS.length];
