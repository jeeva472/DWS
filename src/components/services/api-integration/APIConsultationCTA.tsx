...
import { ArrowRight, Sparkles, Network, CheckCircle2, ShieldCheck, Clock } from "lucide-react";
import { ServicesCTAData } from "@/lib/types/wordpress";

interface APIConsultationCTAProps {
  cta?: ServicesCTAData;
  onOpenConsultation?: () => void;
}

export function APIConsultationCTA({
  cta,
  onOpenConsultation,
}: APIConsultationCTAProps) {
  const shouldReduceMotion = useReducedMotion();
  const eyebrow = cta?.eyebrow || "SYSTEM INTEGRATION";
  const title = cta?.title || "Let's Connect Your Systems";
  const description =
    cta?.description ||
    "Tell us about the tools and platforms you need to connect. We will architect a reliable integration strategy.";
  const buttonText = cta?.buttonText || "Discuss an Integration";
