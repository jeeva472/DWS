...
interface SEOCapabilitiesProps {
  capabilities?: {
    eyebrow?: string;
    title: string;
    description: string;
...
export function SEOCapabilities({ capabilities }: SEOCapabilitiesProps) {
  const shouldReduceMotion = useReducedMotion();
  const [activeCard, setActiveCard] = useState<number | null>(null);

  const eyebrow = capabilities?.eyebrow || "CORE CAPABILITIES";
  const title = capabilities?.title || "Comprehensive search engine optimization.";
  const description =
    capabilities?.description ||
    "We address every dimension of search visibility—from underlying server architecture to high-intent content strategy.";
  const items = capabilities?.items || [];

  return (
    <section id="seo-capabilities" className="py-24 sm:py-32 bg-[#05080a] relative border-t border-white/[0.06] overflow-hidden">
