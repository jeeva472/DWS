...
import { Search, TrendingUp, Target, Code2, Eye, Sparkles } from "lucide-react";
...
export function SEOProcess({ process }: SEOProcessProps) {
  const shouldReduceMotion = useReducedMotion();
  const eyebrow = process?.eyebrow || "OUR SEO FRAMEWORK";
  const title = process?.title || "A systematic, data-informed methodology.";
  const description =
    process?.description ||
    "We avoid guesswork. Every optimization is grounded in search engine documentation and real crawl data.";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {steps.map((step, index) => {
            const IconComponent = PROCESS_ICONS[step.icon] || Search;
