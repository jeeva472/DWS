...
import { ArrowRight, Sparkles, TrendingUp, CheckCircle2, ShieldCheck, Clock } from "lucide-react";
import { ServicesCTAData } from "@/lib/types/wordpress";

interface DigitalMarketingConsultationCTAProps {
  cta?: ServicesCTAData;
  onOpenConsultation?: () => void;
}

export function DigitalMarketingConsultationCTA({
  cta,
  onOpenConsultation,
}: DigitalMarketingConsultationCTAProps) {
  const shouldReduceMotion = useReducedMotion();
  const eyebrow = cta?.eyebrow || "GROWTH STRATEGY SESSION";
  const title = cta?.title || "Let's Build a Smarter Growth Strategy.";
  const description =
    cta?.description ||
    "Schedule a conversation with our digital growth specialists to plan your customer acquisition roadmap.";
  const buttonText = cta?.buttonText || "Start a Conversation";
        ...
        >
          ...
          <div className="mt-12 pt-8 border-t border-white/[0.08] grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-[#7d9287]">
            <div className="flex items-center justify-center gap-2">
              ...
              <span>Tailored CAC Optimization</span>
            </div>
            <div className="flex items-center justify-center gap-2">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <span>Full Funnel Integration</span>
            </div>
          </div>
        </motion.div>
      </div>
