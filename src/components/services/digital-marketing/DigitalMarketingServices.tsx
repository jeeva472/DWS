...
export function DigitalMarketingServices({ services }: DigitalMarketingServicesProps) {
  const shouldReduceMotion = useReducedMotion();
  const [activeCard, setActiveCard] = useState<number | null>(null);

  const eyebrow = services?.eyebrow || "GROWTH CAPABILITIES";
  const title =
    services?.title || "Strategic marketing designed for measurable ROI.";
  const description =
    services?.description ||
    "We focus on qualified audience acquisition, conversion funnels, and data attribution rather than vanity metrics.";
  const items = services?.items || [];

  return (
    <section id="marketing-services" className="py-24 sm:py-32 bg-[#05080a] relative border-t border-white/[0.06] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, index) => {
            const IconComponent = MARKETING_ICONS[index % MARKETING_ICONS.length];
